...
def moveMotors(diff_x, diff_y, panning, tiliting, roi, margin):
    ...
    ROI = margin/1.5
    close_roi = ROI/2
    big_movement_X = int(abs(diff_x/15))
    big_movement_Y = int(abs(diff_y/18))
    med_movement_X = int(abs(diff_x/20))
    med_movement_Y = int(abs(diff_y/20))

    if diff_x > margin*2:
        camera_panning_motor.set_position(camera_panning_motor.get_position() + big_movement_X + 5)
    elif diff_x > margin:
        camera_panning_motor.set_position(camera_panning_motor.get_position() + med_movement_X + 5)
        if diff_x < ROI:
            camera_panning_motor.set_position(camera_panning_motor.get_position() + one_degree + 1)
            if diff_x < close_roi:
                camera_panning_motor.set_position(camera_panning_motor.get_position() + one_degree + 1)
    elif diff_x > ROI:
            camera_panning_motor.set_position(camera_panning_motor.get_position() + one_degree + 3)
            if diff_x < ROI:
                camera_panning_motor.set_position(camera_panning_motor.get_position() + one_degree + 1)
    elif diff_x < 0 - margin*2:
    ...
    if diff_y > margin*2:
        ...
        camera_tilt_motor.set_position(camera_tilt_motor.get_position() + big_movement_Y + 5)
    elif diff_y < 0-margin:
        camera_tilt_motor.set_position(camera_tilt_motor.get_position() + med_movement_Y + 5)
        if diff_y > 0-ROI:
